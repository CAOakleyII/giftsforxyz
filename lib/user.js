...
Meteor.startup(function() {
    if (Meteor.isServer) {
      ...
      Meteor.publish("userWishlist", function () {
        return Meteor.users.find({_id: this.userId}, {fields: {'wishlist': 1}}); // not sure if this is always returning the logged in user
      });
    }

    if(Meteor.isClient){
      Meteor.subscribe('userWishlist')
    }
